...
def panel_creation():
    ...
    button_positions = ['Reset', 'Red', 'Blue', 'Green', 'Erase', 'Save'] # 7 buttons slots
    ...
    for i in range(len(button_positions)):
        ...
        if i==0:    #tamanho do primeiro botão
            button_positions_dict[button_positions[i]] = ((button_x, y_button1),(button_x + button_width_f, y_button2))
            ...
            button_positions_dict[button_positions[i]] = ((button_x, y_button1),(button_x + button_width_c, y_button2))
            button_x += button_width_c + x_space_f

        else:   #tamanho dos botões intermediários (cores)
            button_positions_dict[button_positions[i]] = ((button_x, y_button1),(button_x + button_width_c, y_button2))
            button_x += button_width_c + x_space_c # cada botão será separado por um espaço (button_width)
    ...
    cv2.rectangle(panel, bxy('Reset',0), bxy('Reset',1), (95, 95, 95), -1) # Reset button

    cv2.rectangle(panel, bxy('Green',0), bxy('Green',1), (0, 70, 0), -1) # Green button
    cv2.rectangle(panel, bxy('Blue',0), bxy('Blue',1), (70, 0, 0), -1) # Blue button 
    cv2.rectangle(panel, bxy('Red',0), bxy('Red',1), (0, 0, 100), -1) # Red button  / (top left corner) (bottom right corner)
    cv2.rectangle(panel, bxy('Erase',0), bxy('Erase',1), (35, 35, 35), -1) # Erase button (black color)

    cv2.rectangle(panel, bxy('Save',0), bxy('Save',1), (95, 95, 95), -1) # Save button

    # Adição de texto aos botões (Reset ; Save ; Erase)

    res_text_size = cv2.getTextSize("Reset", cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0] #returns tuple with text x_size,y_size
    #res_text_x = x_res1 + (x_res2 - x_res1 - res_text_size[0]) // 2
    #res_text_y = y_res1 + (y_res2 - y_res1 + res_text_size[1]) // 2
    res_text_x = bxy('Reset',0)[0] + (bxy('Reset',1)[0] - bxy('Reset',0)[0] - res_text_size[0]) // 2 #define posição x do texto em função do botão
    res_text_y = bxy('Reset',0)[1] + (bxy('Reset',1)[1] - bxy('Reset',0)[1] + res_text_size[1]) // 2 #define posição y do texto em função do botão

    sav_text_size = cv2.getTextSize("Save", cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)[0]
    sav_text_x = bxy('Save',0)[0] + (bxy('Save',1)[0] - bxy('Save',0)[0] - sav_text_size[0]) // 2
    sav_text_y = bxy('Save',0)[1] + (bxy('Save',1)[1] - bxy('Save',0)[1] + sav_text_size[1]) // 2
...
def button_function(x, y, img):
    if bxy('Red',0)[0] <= x <= bxy('Red',1)[0] and bxy('Red',0)[1] <= y <= bxy('Red',1)[1]:
        color = (0, 0, 255)  # Red button selected
        radius=2
    elif bxy('Blue',0)[0] <= x <= bxy('Blue',1)[0] and bxy('Blue',0)[1] <= y <= bxy('Blue',1)[1]:
        color = (255, 0, 0)  # Blue button selected
        radius=2
    elif bxy('Green',0)[0] <= x <= bxy('Green',1)[0] and bxy('Green',0)[1] <= y <= bxy('Green',1)[1]:
        color = (0, 255, 0)  # Green button selected
        radius=2
    elif bxy('Erase',0)[0] <= x <= bxy('Erase',1)[0] and bxy('Erase',0)[1] <= y <= bxy('Erase',1)[1]:
        color = (0, 0, 0)  # Erase button selected
        radius=5    
    elif bxy('Reset',0)[0] <= x <= bxy('Reset',1)[0] and bxy('Reset',0)[1] <= y <= bxy('Reset',1)[1]:
        color = (1,1,1) #Reset color
    elif bxy('Save',0)[0] <= x <= bxy('Save',1)[0] and bxy('Save',0)[1] <= y <= bxy('Save',1)[1]:
